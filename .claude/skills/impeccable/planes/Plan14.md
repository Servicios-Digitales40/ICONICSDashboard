...
Fase 2 (modelo): decisión, NO se sube a 14B — se sigue con Qwen3.5-9B-Q4_K_M. Recalibrado y verificado contra el modelo real: `IA_MAX_PASOS` 3→4, `IA_MAX_TOKENS` 512→1024 (el 9B se comía el presupuesto en `reasoning_content` y dejaba la burbuja vacía). `llama-server` pasa a arrancar con `--ctx-size 8192 -ngl 999 -fa on`: sin `--ctx-size` explícito tomaba el contexto de entrenamiento del modelo y la VRAM llegó a 15,5/16,3 GB; con el flag baja a 7,5 GB. 42/42 en verificar-chat.mjs, 65/65 en verificar-herramientas.mjs (incluidos los dos escenarios de diagnóstico contra el modelo real), 51/51 en verificar-backend.mjs.

Fase 5 (reportes PDF) completa y verificada: `backend/package.json` nuevo (antes el backend no tenía ninguno) con `pdfkit` y `svg-to-pdfkit`, cargadas con `await import()` dentro de `generar_reporte` y nunca en la cabecera de `herramientas.mjs` (`backend/ia/reporte.mjs` sí las importa arriba, porque ese módulo entero sólo se carga perezosamente). `GET /api/reportes` va por query string (`?id=`) y no por `/api/reportes/:id`: el router no soporta segmentos de ruta a propósito. `leerHistoriaLarga` se generalizó a `leerSerieEnRango` (rango explícito, no sólo "N días desde ahora") y `resolverVentana` ganó un tope configurable (`maxHoras`) para que el reporte admita hasta ~31 días sin tocar el límite de 7 días del resto de herramientas. Reportes viejos se purgan solos (perezoso, en cada escritura, mismo criterio que la caché de lote). Probado de punta a punta contra el modelo real con `ICONICS_FAKE=true`: el asistente llamó a `generar_reporte`, entregó el adjunto con el enlace de descarga y el PDF se descargó válido (cabeceras correctas, 4 gráficos + tabla). 72/72 en verificar-herramientas.mjs, 54/54 en verificar-backend.mjs, sin regresiones en verificar-chat.mjs (42) ni verificar-transporte-falso.mjs (12).

Alcance honesto de la Fase 5: probado contra `ICONICS_FAKE=true`, no contra el servidor real — sigue pendiente medir con historia real (§7.3), igual que el resto de fases que no tocan planta.

Bug encontrado y corregido de paso, no introducido por esta fase: `chat.mjs` armaba el evento SSE del adjunto como `{ tipo: 'adjunto', ...adjunto }`, y como el propio adjunto ya trae un campo `tipo` ('grafico', 'reporte'), el spread lo sobrescribía — el evento nunca llegaba como `tipo: "adjunto"`, así que `useAsistente.js` jamás lo enrutaba a `onAdjunto` y el adjunto se perdía en silencio, sin excepción en ningún lado. Esto llevaba dormido desde que existe `grafico_de_senal`: los gráficos tampoco se habían pintado nunca en el panel real, y nada lo detectó porque ninguna prueba ejercitaba `leerFlujo` contra un SSE de verdad con un adjunto dentro. Se corrigió anidando el adjunto (`{ tipo: 'adjunto', adjunto }`) en vez de mezclarlo por spread, y el panel ahora reparte por `adjunto.formato` ('svg' → gráfico inline, 'pdf' → botón de descarga vía el nuevo `ReporteDescarga`). Confirmado con curl contra el modelo real: el gráfico y el reporte llegan y se sirven bien. 30/30 en la suite de frontend de `asistente/`, build de producción limpio.
