...
/*
 * Aquí vivían los cuatro lectores —`pdfjs`, el casero de respaldo, el de
 * `.docx` y sus ayudantes—, unas 350 líneas. Se mudaron a
 * `extraccion.worker.mjs` en el Plan 22 F2 (SEG-10) sin tocar una sola línea
 * de su lógica: corrían en el hilo que atiende las lecturas de planta, y un
 * PDF malformado congelaba el tablero entero mientras se procesaba. La
 * cabecera de aquel archivo lleva el razonamiento completo.
 *
 * Lo que se queda aquí es lo que juzga el resultado —`pareceTexto`— y no la
 * mecánica de sacarlo: quién decide si un texto entra al índice es el índice.
 */
 ...
 * @param {{msMaximo?:number, maxPaginas?:number, maxCaracteres?:number}} [opciones.topes]
 *        Los límites de la extracción en hilo (Plan 22 F2). Por defecto los de
 *        `extraccion.mjs`, que son los de producción. El verificador los baja
 *        para provocar cada corte sin necesitar un PDF patológico de verdad —
 *        probar un tope de 1500 páginas exigiría fabricar 1501, y lo que hay
 *        que comprobar es el mecanismo, no la cifra.
 */
export function createIndiceDocumentos({
  ...
  /**
   * Archivos que SÍ se indexaron, pero no enteros (Plan 22 F2).
   *
   * Van aparte de `ilegibles` porque el arreglo es distinto y la lectura
   * también: un ilegible no aporta nada al índice y hay que convertirlo; un
   * parcial aporta lo que entró, y lo que falta es lo que hay que decidir si
   * importa. Meterlos en la misma lista obligaría a leer el `motivo` para
   * saber cuál es cuál.
   */
  let parciales = []
  ...
  /**
   * Devuelve `{ paginas, parcial }`. `parcial` es el motivo por el que el
   * archivo entró recortado, o `null`.
   *
   * Se devuelve en vez de anotarse aquí porque tiene que sobrevivir a la
   * caché: un archivo recortado SÍ se cachea —lo que entró es bueno— y en la
   * siguiente `recargar()` no vuelve a pasar por esta función. Si el aviso se
   * escribiera aquí, desaparecería de la pantalla en cuanto otro archivo
   * disparara una recarga, que es exactamente el fallo que `ilegibles` ya
   * tuvo una vez y que su prueba vigila.
   */
  async function leerDocumento(ruta, archivo) {
    const ext = extname(archivo).toLowerCase()

    if (!NECESITAN_HILO.has(ext)) {
      const contenido = await readFile(ruta, 'utf8')
      // Un .txt no tiene páginas: se cita como página 1, que es lo honesto.
      return { paginas: contenido ? [{ pagina: 1, texto: contenido }] : [], parcial: null }
    }

    const { paginas, truncado, motivoRespaldo, error } = await extraerTexto({ ruta, ext, ...topes })

    // Firma que no cuadra, hilo cortado por tiempo, hilo caído: el motivo ya
    // viene redactado desde `extraccion.mjs`, que es quien sabe cuál fue.
    if (error) {
      ilegibles.push({ archivo, motivo: error })
      return { paginas: [], parcial: null }
    }

    if (motivoRespaldo) {
      logger.warn('pdfjs no pudo abrir un PDF; se usó el lector de respaldo', {
        archivo, error: motivoRespaldo,
      })
    }

    /*
     * Un archivo recortado se INDEXA —lo leído es bueno— pero no se calla.
     * Sin este aviso, un manual de 4000 páginas del que entraron 1500 se ve
     * en la pantalla de Documentación exactamente igual que uno completo, y
     * el asistente contestaría «no aparece en el manual» sobre algo que sí
     * está, en la página 2300 que nunca se leyó.
     */
    const parcial = truncado
      ? truncado.motivo === 'paginas'
        ? `sólo se indexaron ${truncado.leidas} de sus ${truncado.total} páginas (tope: ${truncado.tope})`
        : `se cortó al pasar de ${truncado.tope.toLocaleString('es-MX')} caracteres, en la página ${truncado.leidas}`
      : null

    const buenas = paginas.filter(p => pareceTexto(p.texto))
    if (buenas.length) return { paginas: buenas, parcial }
    ...
    ilegibles.push({
      archivo,
      motivo: ext !== '.pdf'
        ? 'no se pudo leer el contenido; ¿es un .doc antiguo renombrado, o está protegido con contraseña?'
        : paginas.length
    ...
    cargando = (async () => {
      ...
      for (const [archivo, h] of mapaHuellas) {
        ...
        try {
          ...
          const { paginas, parcial } = await leerDocumento(ruta, archivo)
          const fragmentos = []
          ...
          if (fragmentos.length) {
            archivosNuevos.set(archivo, { huella: h, fragmentos, parcial })
      ...
      // Desde la caché, no desde esta pasada: un archivo recortado que hoy se
      // reutilizó sin releerse sigue estando recortado. Ver `leerDocumento`.
      parciales = [...archivosProcesados]
        .filter(([, e]) => e.parcial)
        .map(([archivo, e]) => ({ archivo, motivo: e.parcial }))
      cargado = true
      ...
      logger.info(
        `Documentación indexada: ${archivosProcesados.size} archivo(s) (${reutilizados} sin ` +
          ...
          (ilegibles.length
            ...
            : '') +
          (parciales.length
            ? `. ${parciales.length} archivo(s) entraron RECORTADOS: ` +
              parciales.map(p => `${p.archivo} (${p.motivo})`).join('; ')
            : ''),
        {
          carpeta,
  ...
  function estado() {
    ...
    return {
       ...
       * que hay que decidir es si lo que quedó fuera importaba.
       */
      parciales,
      /*
