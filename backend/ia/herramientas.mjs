...
export const DEFINICIONES = [
  {
    ...
    function: {
      ...
      description:
        'Lo que YA se sabe confirmado de esta instalación: datos que alguien verificó alguna vez ' +
        'y que no se deducen mirando el servidor —cuántos sensores hay de verdad, cómo se llama ' +
        'exactamente un grupo del historiador, qué tensión nominal aplica—. Llámala al empezar a ' +
        'hablar de un sistema que no conoces, o cuando una respuesta dependa de un detalle de la ' +
        'instalación que podrías estar suponiendo. Cada hecho viene con su ORIGEN: quién lo ' +
        'confirmó y cuándo. Cítalo cuando lo uses.',
      parameters: {
        type: 'object',
        properties: {
          sistema: {
  ...
  },
  {
    ...
    function: {
      name: 'recordar_hecho',
      description:
        'Guarda un dato de la instalación que EL USUARIO acaba de confirmarte, para que esté ' +
        'disponible en las siguientes conversaciones. Úsala cuando te digan algo como "el sensor ' +
        'S3 es de 100 mV/g", "la tensión es de 208", "el rodamiento intermedio es un 6206". ' +
        'IMPORTANTE: sólo para lo que una PERSONA afirma, nunca para lo que tú deduzcas de los ' +
        'datos —para eso está proponer_regla—. Necesita `origen`: quién lo dijo y cuándo. Sin ' +
        'eso no se guarda, porque dentro de un mes nadie podrá distinguir un dato de planta de ' +
        'una conjetura tuya bien redactada.',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      name: 'proponer_regla',
      description:
        'Deja ANOTADA una regla de riesgo que crees que faltaría, para que una persona la revise. ' +
        'NO crea la regla ni hace que el sistema empiece a vigilar eso: queda pendiente de ' +
        'aprobación. Úsala cuando observes en los datos un patrón que hoy no está cubierto —una ' +
        'combinación de señales que parece peligrosa y de la que nadie avisa—. La evidencia tiene ' +
        'que llevar las CIFRAS que la sostienen, no sólo la idea. Cuando la uses, dile al usuario ' +
        'que la has anotado para revisión y que ejecute `node scripts/revisar-propuestas.mjs`; ' +
        'NUNCA le digas que has creado una regla ni que el sistema ya avisa de eso.',
      parameters: {
        type: 'object',
        properties: {
