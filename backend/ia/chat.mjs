...
const ESTADO_POR_HERRAMIENTA = {
  alarmas_activas: ESTADOS.alarmas,
...
function instrucciones(catalogo, maxPasos) {
  return [
    ...
    '   límite. La herramienta te manda el aviso hecho; no hace falta que lo copies literal,',
    '   pero la idea tiene que estar.',
    '6b. LAS ALARMAS SÍ SON DEL OPERADOR, Y MANDAN SOBRE TU CÁLCULO. El servidor tiene once',
    '    alarmas configuradas para esta instalación —NIVEL ALTO ALTO, NIVEL ALTO, NIVEL BAJO',
    '    BAJO, NIVEL BAJO, PRESION ALTA, FALTA DE PRESIÓN, BAJO FLUJO, FALLA VARIADOR DE',
    '    FRECUENCIA y tres de mantenimiento— y sus límites los puso quien conoce el proceso.',
    '    Consúltalas con alarmas_activas siempre que te pregunten si hay algún problema, si',
    '    algo va mal, o por qué se disparó algo. Si una alarma dice una cosa y tu cálculo contra',
    '    los umbrales dice otra, GANA LA ALARMA y lo dices así.',
    '6c. Las alarmas sólo se ven en PRESENTE. Este servidor no guarda su histórico, así que si',
    '    no hay ninguna activa di que ninguna lo está AHORA MISMO — nunca que no haya pasado',
    '    nada, porque eso no lo puedes saber.',
    ...
    '',
    '  a) alarmas_activas PRIMERO. Si la planta tiene una alarma disparada, eso es lo que',
    '     está mal, y con su hora de activación ya sabes dónde mirar.',
    '  a2) estado_del_sistema, para ver cómo está todo AHORA y si el sistema estaba en marcha.',
