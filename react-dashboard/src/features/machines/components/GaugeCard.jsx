 ...
 * ThemeProvider, así que se adapta a claro/oscuro con el resto de la app: no
 * hay colores literales de marca, `palette(theme, dark)` traduce los tokens
 * globales a los nombres semánticos que usa este componente.
 *
 * El estado es una clave canónica del dominio (`running`, `standby`, …); la
 * etiqueta y el color salen de `lib/domain/estado.js` y aquí solo queda el
 * icono. Cualquier métrica puede ser `null` y se pinta «—», nunca 0.
 */
...
/**
 * Icono de cada estado, por clave canónica del dominio. La etiqueta y el color
 * los pone `lib/domain/estado.js`, para que no puedan divergir.
 *
 *   ✔ verde → operando                ⚡ ámbar → sin comunicación con el PLC
 *   ⏸ gris  → stand by                ⛔ rojo  → alarma activa
 *   ⚙ azul  → set-up en curso         ⃠ tenue → sin dato
 */
const ICONO_ESTADO = {
  running: CircleCheck,
  standby: PauseCircle,
...
function Gauge({ valor, P }) {
  // Sin medición el arco va vacío y en tono apagado: la aguja a 0 en rojo
  // diría «OEE del 0 %», que es una afirmación y no un hueco.
...
export default function GaugeCard({
  ...
  // OEE = (D × R × C) / 10000, en %. Los tres factores llegan como
  // porcentajes, y dividir entre 10000 devuelve el producto a la escala 0–100.
  //
  // Se prefiere el valor del servidor, que aplica la misma fórmula sobre
  // lecturas instantáneas; recomponerlo aquí mezclaría factores de ciclos
  // distintos.
  const valor = useMemo(() => {
...
/* Aquí vivía `AreaDemo`, que enseñaba un equipo por estado. Se retiró porque
   estaba escrita en el vocabulario antiguo de estados; su función la cumple
