/**
 * El cajón «Manuales» — qué documentos alimentan el índice del asistente, y
 * qué sabe de verdad extraer de cada uno.
 *
 * ── POR QUÉ ES UN CAJÓN Y NO UNA PANTALLA (PLAN 20 FASE 3) ─────────
 *
 * Fue la vista «RAG · Documentación». La rama `Asistente` no tiene rutas: hay
 * una sola vista, el chat, y lo que haya que enseñar se enseña dentro de ella.
 * Esto sobrevivió al borrado de las otras dieciocho pantallas por el criterio
 * de `docs/PLAN-20-ASISTENTE.md` §1 —lo que ALIMENTA al asistente se queda—
 * y porque **es la capacidad 7 del encargo**: anexar los manuales desde la UI.
 * Es el único camino por el que entra conocimiento externo al sistema.
 *
 ...
 *
 * Mismo criterio que el cajón hermano de casos, y por el mismo motivo (ver
 * «ARCHIVAR: LA ÚNICA BAJA QUE EXISTE» en `@shared/eva/comun/aprendizaje.js`).
 * Archivar mueve el manual fuera de lo que el índice recorre —deja de
 * contestar preguntas, sin perder el archivo— y es la única baja que existe.
 */
...
export default function CajonManuales() {
